	...
	$.fn.crossword = function (entryData, user_head, next_id) {
				...
				*/
				calcCoords: function () {
					...
					*/
					...
					$('#' + puzz.data[i].orientation).append('<li tabindex="1" data-ori="' + puzz.data[i].orientation +'" data-position="' + puzz.data[i].position  + '">' + puzz.data[i].position + ". " + puzz.data[i].clue + '</li>');
				}
			...
			buildEntries: function () {
				var puzzCells = $('#puzzle td'),
					...
					for (var i = 0; i < entries[x - 1].length; ++i) {
						...
						// console.log(puzz.data[x - 1].orientation)
						light.attr("data-ori", puzz.data[x - 1].orientation)
						...
						clue_entry[x] = [puzz.data[x-1].position, puzz.data[x-1].orientation] 
		...
		var util = {
			...
			highlightClue: function () {
				...
				$(clueLiEls + '[  data-position=' + (clue_entry[activePosition][0]) + '][data-ori="' + (clue_entry[activePosition][1]) +'"]').addClass('clues-active');
				if (mode === 'interacting') {
		...
		function randomSolve() {
			...
			t = puzz.data[clue_entry[random_one - 1][0]]
			console.log(puzz.data)
			console.log(clue_entry)
			console.log(clue_entry[random_one], random_one)
			console.log(puzz.data[clue_entry[random_one][0]])
			random_letter = t.answer.split('')
			td = (document.querySelectorAll(`.position-${random_one}`))
		...
		function solveWord() {
			...
			td = document.querySelectorAll(`.position-${activePosition}`)
